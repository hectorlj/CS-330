...
function type_of_expr(ast::IsZeroNode, env::TypeEnvironment)
    param = type_of_expr(ast.arg, env)
    if param == NumType()
      return BoolType()
    end
    throw(LispError("IsZeroNode takes a NumType"))
end
...
function type_of_expr(ast::NIsEmptyNode, env::TypeEnvironment)
  if type_of_expr(ast.list, env) != NListType()
    throw(LispError("NIsEmptyNode takes a listType"))
  end
  return BoolType()
end
...
function type_of_expr(ast::NFirstNode, env::TypeEnvironment)
  if type_of_expr(ast.list, env) != NListType()
    throw(LispError("Bad arg for NFirstNode"))
  end
  return NumType()
end

function type_of_expr(ast::NConsNode, env::TypeEnvironment)
  if type_of_expr(ast.f) != NumType()
    throw(LispError("NConsNode takes a NumType"))
  elseif type_of_expr(ast.r) != NListType()
    throw(LispError("NConsNode takes a list"))
  end
  return NListType()
end
...
function type_of_expr(ast::IfBNode, env::TypeEnvironment)
  if type_of_expr(ast.cond, env) != BoolType()
    throw(LispError("IfbNode takes a boolType"))
  end
  tbType = type_of_expr(ast.nzerobranch, env)
  fbType = type_of_expr(ast.zerobranch, env)
  if !same_type(tbType, fbType)
    throw(LispError("Types of branches in IfBNode do not match"))
  end
  return tbType
end

function type_of_expr(ast::FuncDefNode, env::TypeEnvironment)
  newEnvironment = ExtendedTypeEnv(ast.formal_parameter, ast.formal_type, env)
  body = type_of_expr(ast.body, newEnvironment)
  return FuncType(ast.formal_type, body)
end

function type_of_expr(ast::FuncAppNode, env::TypeEnvironment)
  func_type = type_of_expr(ast.fun_expr)
  arg_type = type_of_expr(ast.arg_expr)
  if arg_type != func_type.arg_type
    throw(LispError("FuncAppNode argument and param types must match"))
  end
  return func_type.result_type
end

function type_of_expr(ast::WithNode, env::TypeEnvironment)
    arg_type = type_of_expr(ast.binding_expr)
    newEnvironment = ExtendedTypeEnv(ast.sym, arg_type, env)
    bodyType = type_of_expr(ast.body, newEnvironment)
    return bodyType
end
